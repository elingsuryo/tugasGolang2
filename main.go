...
func takeOrder(restaurant *Restaurant, ch chan<- Order) {
	...
	for {
		...
		if menuItem, ok := validateOrderItem(restaurant, itemName); ok {
			...
			fmt.Scanln(&itemQty)
			order.MenuItems = append(order.MenuItems, MenuItem{Name: menuItem.Name, Price: menuItem.Price * float64(itemQty)})
			order.Total += menuItem.Price * float64(itemQty) // Menghitung total harga berdasarkan jumlah item
		} else {
